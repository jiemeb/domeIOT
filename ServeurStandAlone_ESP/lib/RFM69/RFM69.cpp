...
bool RFM69::initialize(uint8_t freqBand, uint8_t nodeID, uint8_t networkID)
{
  const uint8_t CONFIG[][2] =
  {
    ...
    /* 0x2F */ { REG_SYNCVALUE2, 0x2D },      // attempt to make this compatible with sync1 byte of RFM12B lib
    /* 0x30 */ { REG_SYNCVALUE3, networkID }, // NETWORK ID
...
{
  //  Serial.printf("Adresse %x long %d request %d send %d \n",toAddress,bufferSize,requestACK,sendACK);
 ...
 //   CTLbyte = RFM69_CTL_SENDACK;
	toAddress = 0x80 | toAddress ; // C Bit
  else if (requestACK)
 //   CTLbyte = RFM69_CTL_REQACK;
	toAddress = RFM69_CTL_REQACK | toAddress ; // A Bit
 else if (!(toAddress & RFM69_CTL_DEST)) // broadcast message
  {toAddress = (toAddress & ~0x1F) | TARGETID ; }

//Serial.printf("Adresse %x long %d \n",toAddress,bufferSize);

  // write to FIFO
  select();
/* try to compatible rfm12B Jeelib
...
*/
 ...
 crc = ~0;
   //Serial.printf("   [CRC:%x]\n",crc);
  crc=rfm_crc16_update(crc,  bufferSize+4);
  crc=rfm_crc16_update(crc,  toAddress);
  SPI.transfer(bufferSize + 4); // Lenght of Data + Header
  SPI.transfer(toAddress);
 // SPI.transfer(_address); // not transmit our address
 // SPI.transfer(CTLbyte);
  for (uint8_t i = 0; i < bufferSize; i++)
  {
    SPI.transfer(((uint8_t*) buffer)[i]);
  crc=  rfm_crc16_update(crc, ((uint8_t*) buffer)[i]);
...
//       || PAYLOADLEN < 3) // address situation could receive packets that are malformed and don't fit this libraries extra fields
  if(!(!(TARGETID & RFM69_CTL_DEST) || ((TARGETID & 0x1F ) == _address) || TARGETID == RF69_BROADCAST_ADDR) // match this node's address, or broadcast address or anything in promiscuous mode
       || PAYLOADLEN < 2) // address situation could receive packets that are malformed and don't fit this libraries extra fields
    {
      PAYLOADLEN = 0;
      unselect();
      receiveBegin();
...
//    PAYLOADLEN +=2 ;
    ...
    crc = ~0;
     crc=rfm_crc16_update(crc,  PAYLOADLEN);
     crc=rfm_crc16_update(crc,  TARGETID);

SENDERID = TARGETID & 0x1F  ;

    for (uint8_t i = 0; i < DATALEN ; i++) // pour CRC
    {
      DATA[i] = SPI.transfer(0);
      crc=rfm_crc16_update(crc,  DATA[i]);
    }
    crc=rfm_crc16_update(crc, SPI.transfer(0)); //CRC check LSB
    crc=rfm_crc16_update(crc, SPI.transfer(0));//CRC check MSB
    //Serial.print("CRC ");
