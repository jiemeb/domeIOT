...
#define RF_SLEEP_MODE   0x8206
#define RF_WAKEUP_MODE  0x8206*/
...
static void rf12_interrupt () {
    ...
    if (rxstate == TXRECV) {				// interrupt receive
    ...
    } else {						// Interrupt transmit
        uint8_t out;

        if (rxstate < 0) {
            uint8_t pos = 3 + rf12_len_buf + rxstate++;
            out = rf12_buf[pos];
            ...
            switch (rxstate++) {
                ...
                case TXSYN2: out = group; rxstate = - (2 + rf12_len_buf); break;
...
//    if (rxstate == TXRECV && (rxfill >= rf12_len + 5 || rxfill >= RF_MAX)) {
    if (rxstate == TXRECV && (rxfill > rf12_len  || rxfill >= RF_MAX)) {
        rxstate = TXIDLE;
...
// caller should validate!    if (frequency < 96) frequency = 1600;
  ...
  rf12_xfer(0xC623); // approx 9600 bps, i.e. 10000/29/(1+6) Kbps  
//     rf12_xfer(0xC606); // approx 49.2 Kbps, i.e. 10000/29/(1+6) Kbps
//     rf12_xfer(0xC60B); // approx 28200 bps, i.e. 10000/29/(1+6) Kbps

    rf12_xfer(0x94A2); // VDI,FAST,134kHz,0dBm,-91dBm 

// Test JM    rf12_xfer(0x94A3); // VDI,FAST,134kHz,0dBm,-85dBm
